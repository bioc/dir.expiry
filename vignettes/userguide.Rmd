...
cat(readLines(file.path(base.path, "1.0.0_dir.expiry")), sep="\n")
```

# Deleting old caches

`touchDirectory()` will automatically delete all expired caches that it finds in the base directory.
...
The deletion procedure will attempt to acquire an exclusive lock prior to deletion.
...
This allows multiple processes to read from the cache at the same time, only being blocked when one process needs to perform a write operation.
